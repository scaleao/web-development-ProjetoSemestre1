...
try {
  ...
  if (!move_uploaded_file(
      ...
      sprintf('../../users/'.$usuario["email"].'/fotoperfil/%s.%s',
  ...
  $itens = glob("../../users/".$usuario["email"]."/fotoperfil/*");
  ...
  echo ($nomeAtual); // ---> Nome certo para jogar no banco ./users/[a-z][.[a-z]{3}]

// Exemplo para renomar o arquivo que acabei não usando:
//  rename($nomeAtual, "../../users/".$usuario["email"].$nomeFotoPerfil);
  $messages_sucess .= "<li>File is uploaded successfully.</li>";
